  ...
  return (
    <div className="p-4 sm:p-6 md:p-8">
      <div className="flex items-center justify-between mb-6 md:mb-8">
        <div>
          <h1 className="text-xl sm:text-2xl font-headline font-bold text-inverse-surface">Blog</h1>
      ...
      <div className="sm:hidden space-y-3">
        {posts.map((post) => (
          <div key={post.id} className="bg-white rounded-xl border border-slate-100 p-3 flex items-center gap-3 shadow-sm">
            {post.coverImage
              ? (
            ...
            <div className="flex-1 min-w-0">
              <p className="font-medium text-inverse-surface truncate text-sm">{post.title}</p>
              <p className="text-[10px] text-slate-400 truncate font-mono mt-0.5">/{post.slug}</p>
              <div className="flex items-center gap-2 mt-1.5">
                <span className={`px-2 py-0.5 rounded-full text-xs font-bold ${post.status === 'published' ? 'bg-green-100 text-green-700' : 'bg-yellow-100 text-yellow-700'}`}>
        ...
        {posts.length === 0 && (
          <p className="text-center text-slate-400 py-12 text-sm">
            ...
            <Link href="/admin/blog/new" className="text-primary-container font-medium hover:underline">Crear el primero</Link>
          </p>
        )}
      </div>
          ...
          <tbody>
            {posts.map((post) => (
              <tr key={post.id} className="border-b border-slate-50 hover:bg-[#f8fafc] transition-colors">
                <td className="px-4 sm:px-5 py-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="text-xs text-slate-400 mt-0.5 truncate max-w-44 sm:max-w-64 md:max-w-72">/{post.slug}</p>
                    </div>
                  </div>
                </td>
                ...
                <td className="px-4 sm:px-5 py-4">
                  <span className={`px-2.5 py-1 rounded-full text-xs font-bold ${post.status === 'published' ? 'bg-green-100 text-green-700' : 'bg-yellow-100 text-yellow-700'}`}>
                    {post.status === 'published' ? 'Publicado' : 'Borrador'}
                  </span>
                </td>
                <td className="px-4 sm:px-5 py-4">
            ...
            {posts.length === 0 && (
              <tr>
                <td colSpan={5} className="px-5 py-12 text-center text-slate-400 text-sm">
                  No hay artículos todavía.{' '}
                  <Link href="/admin/blog/new" className="text-primary-container font-medium hover:underline">
                    Crear el primero
                  </Link>
                </td>
              </tr>
